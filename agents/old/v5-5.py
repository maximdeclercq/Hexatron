...
def get_best_move_with_ray_casting(board: np.ndarray, player_position: np.ndarray, player_rotation: int, opponent_position: np.ndarray, opponent_rotation: int, playable_moves: list) -> int:
    ...
    for move in playable_moves:
        ...
        distance_to_opponent = calculate_distance_between(new_board, ray_position, opponent_position)
        if distance_to_opponent is not None:
            best_area_with_opponent += ((ray_length >= distance_to_opponent) +  (ray_length > distance_to_opponent)) / 64
