...
def calculate_win_probability(board: np.ndarray, player: object, opponent: object, move: int, max_time: int) -> float:
    ...
    t0 = time.time()
    while time.time() - t0 < 0.85 * max_time:
        new_board = board.copy()
        new_player_position, new_player_rotation = player.position.copy(), player.rotation
        new_opponent_position, new_opponent_rotation = opponent.position.copy(), opponent.rotation
        ...
        while True:
            ...
            for j in range(-2, 3):
                new_player_position = calculate_new_position(new_player_position, new_player_rotation, move if first else j)
                new_opponent_position = calculate_new_position(new_opponent_position, new_opponent_rotation, j)
            ...
            new_player_position, new_player_rotation = random.choice(player_targets)
            new_opponent_position, new_opponent_rotation = random.choice(opponent_targets)
...
def get_best_move(board: np.ndarray, player: object, opponent: object) -> int:
    ...
    dist = calculate_distance_between(board, player.position, opponent.position)

    if len(unique_areas) == 1 and dist >= 0:
        ...
        best_win_probabilities = [(win_probability, move) for win_probability, move in best_win_probabilities if win_probability > 0.9]
        ...
        other_win_probabilities = [(win_probability, move) for win_probability, move in other_win_probabilities if win_probability > 0.9]
        if len(best_win_probabilities):
            return random.choice(best_win_probabilities)[1]
        elif len(other_win_probabilities):
            return random.choice(other_win_probabilities)[1]
        return random.choice(best_moves)
    else:
        # fill area
